...
- **Pixel font**: **Geist Pixel**, self-hosted at `app/fonts/GeistPixel.ttf` via `next/font/local` (NOT next/font/google — Geist Pixel isn't in its font list yet, it errors). Exposed as `--font-pixel` / `.font-pixel`. Used SMALL for HUD (coin/XP counters, tab labels, `+5` coin-pop, "3D" toggle). Body/labels stay Inter; don't set long text in pixel.
