...
const SIZES = {
  sm: { px: 40, icon: 20, badge: "text-[11px] h-[18px] min-w-[18px] -bottom-0.5 -right-0.5" },
  md: { px: 56, icon: 28, badge: "text-[13px] h-[22px] min-w-[22px] -bottom-0.5 -right-0.5" },
  lg: { px: 84, icon: 42, badge: "text-[17px] h-[28px] min-w-[28px] -bottom-1 -right-1" },
  xl: { px: 116, icon: 58, badge: "text-[20px] h-[34px] min-w-[34px] -bottom-1 -right-1" },
};
...
export default function PetAvatar({
  ...
  return (
    <div className="relative shrink-0 select-none" style={{ width: s.px, height: s.px }} aria-label={`${pet.name} the ${pet.breed}`}>
      <div
        className="flex h-full w-full items-center justify-center rounded-full text-white shadow-[inset_0_1px_1px_rgba(255,255,255,0.45),inset_0_-2px_6px_rgba(0,0,0,0.12),0_2px_6px_rgba(0,0,0,0.1)]"
        style={{ background: `linear-gradient(150deg, ${pet.gradient[0]}, ${pet.gradient[1]})` }}
      >
        <Icon name="paw" size={s.icon} strokeWidth={1.6} className="opacity-95 drop-shadow-[0_1px_1px_rgba(0,0,0,0.15)]" />
      </div>
